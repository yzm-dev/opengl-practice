...
struct CameraController
{
    ...
    CameraController(int window_width,
                     int window_height,
                     glm::vec3 camera_position = glm::vec3(0.0f, 0.0f, 3.0f))
        : camera(camera_position),
          last_x(static_cast<float>(window_width) / 2.0f),
          last_y(static_cast<float>(window_height) / 2.0f)
    {
    }
...
static void processInput(GLFWwindow *window, CameraController &camera_controller)
{
...
static void mouseCallback(GLFWwindow *window, double xpos_in, double ypos_in)
{
